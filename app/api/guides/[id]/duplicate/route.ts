import { NextResponse, type NextRequest } from 'next/server'
import { createClient, getUserSafe } from '@/lib/supabase/server'
import { createAdminClient } from '@/lib/supabase/admin'

// POST /api/guides/[id]/duplicate — clone a guide as a new draft
export async function POST(
  _request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  const { id } = await params
  const supabase = await createClient()
  const { user } = await getUserSafe(supabase)
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })

  const { data: profile } = await supabase.from('profiles').select('role').eq('id', user.id).single()
  if (!profile || !['admin', 'author'].includes(profile.role)) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  }

  const admin = createAdminClient()
  const { data: source } = await admin
    .from('guides')
    .select('title, category, excerpt, content, image_url, reading_time_minutes')
    .eq('id', id)
    .single()

  if (!source) return NextResponse.json({ error: 'Source guide not found' }, { status: 404 })

  const baseTitle = `${source.title} (copy)`.slice(0, 120)
  const { generateUniqueSlug } = await import('@/lib/slug')
  const slug = await generateUniqueSlug(admin, 'guides', baseTitle)

  const { data: newArticle, error } = await admin
    .from('guides')
    .insert({
      author_id: user.id,
      slug,
      title:                baseTitle,
      category:             source.category,
      excerpt:              source.excerpt,
      content:              source.content,
      image_url:            source.image_url,
      reading_time_minutes: source.reading_time_minutes,
      status: 'draft',
    })
    .select('id')
    .single()

  if (error) {
    console.error('Guide duplicate failed:', error)
    return NextResponse.json({ error: `Duplicate failed: ${error.message}` }, { status: 500 })
  }

  return NextResponse.json({ article: newArticle }, { status: 201 })
}
