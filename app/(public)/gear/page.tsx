...
export const metadata: Metadata = {
 ...
 description: 'Every product Boss Daddy has personally bought, tested, and stands behind — sorted by rating. The only gear list where every pick is earned, not sponsored.',
 openGraph: {
 title: 'Boss Daddy Gear List — Top-Rated Dad Gear',
 description: 'Every product personally bought, tested, and rated. The only gear list where every pick is earned, not sponsored.',
 images: [{ url: '/api/og?title=Boss+Daddy+Gear+List&type=review', width: 1200, height: 630 }],
 },
 twitter: {
...
export default async function GearListPage({ searchParams }: Props) {
 ...
 key={r.id}
 href={`/reviews/${r.slug}`}
 className="group flex flex-col bg-gray-900 rounded-2xl overflow-hidden transition-all duration-200"
 >
 {r.image_url ? (
