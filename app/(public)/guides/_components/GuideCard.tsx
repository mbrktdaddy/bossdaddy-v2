...
export default function GuideCard({ guide: a, priority = false }: { guide: GuideRow; priority?: boolean }) {
  ...
  return (
    <Link
      href={`/guides/${a.slug}`}
      className="group flex flex-col bg-surface rounded-xl overflow-hidden border border-soft shadow-lg shadow-black/30 hover:border-copper hover:shadow-xl hover:shadow-black/50 hover:-translate-y-1 transition-all duration-200"
    >
      {a.image_url ? (
