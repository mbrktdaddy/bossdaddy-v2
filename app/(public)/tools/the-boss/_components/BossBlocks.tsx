import type { Block, ProductBlock, SourceBlock } from '@/lib/boss/types'
...
// group into the second-class ResearchedList; live-search sources go last.
// Future action-tool blocks (confirm preview / result card) slot in here with no
// change to callers.
...
export default function BossBlocks({ items, query }: { items: Block[]; query?: string }) {
  ...
  const sources = unique.filter((b): b is SourceBlock => b.kind === 'source')
