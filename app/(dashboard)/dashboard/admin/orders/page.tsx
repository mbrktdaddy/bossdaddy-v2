...
export default async function AdminOrdersPage() {
  ...
  if (error) {
    return (
      <div className="p-8">
        <p className="text-danger-ink text-sm">Failed to load orders: {error.message}</p>
      </div>
    )
  }
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!order.confirmation_email_sent_at && order.confirmation_email_attempts > 0 && (
                      <span
                        ...
                        className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-danger-bg text-danger-ink"
                      >
