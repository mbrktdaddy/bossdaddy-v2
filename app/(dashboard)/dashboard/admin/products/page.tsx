          ...
          {rows.map((p) => (
            ...
            >
              ...
              <div className="shrink-0 flex items-center gap-2 text-xs text-prose-faint">
                {p.affiliate_url ? <span className="px-2 py-1 rounded-md bg-accent-tint/40 text-accent-text-soft border border-accent-border/40">{p.store === 'amazon' ? 'Amazon' : p.store === 'other' ? (p.custom_store_name ?? 'Other') : p.store}</span> : null}
