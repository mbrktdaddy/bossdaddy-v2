...
export default async function MyGuidesPage({ searchParams }: Props) {
    ...
    <div className="p-4 sm:p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <div>
          ...
          <p className="text-prose-faint text-sm mt-1">Guides, how-tos, and dad wisdom</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
        {statCards.map((s) => {
          ...
          const href = isActive && s.filterKey !== null
            ? '/dashboard/guides'
            : s.filterKey ? `/dashboard/guides?filter=${s.filterKey}` : '/dashboard/guides'
          return (
            <Link
              key={s.label}
              href={href}
      ...
      {filter && (
        <div className="flex items-center gap-2 mb-4">
          ...
          <Link href="/dashboard/guides" className="text-xs text-prose-faint hover:text-prose-muted underline">clear</Link>
        </div>
      )}
      ...
      {!displayed?.length && !filter && (
        <div className="text-center py-24 border border-dashed border-soft rounded-2xl">
          ...
          <Link href="/dashboard/guides/new" className="text-accent-text-soft hover:text-orange-300 text-sm">
