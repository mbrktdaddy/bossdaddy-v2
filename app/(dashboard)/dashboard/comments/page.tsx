...
const STATUS_TABS = [
  { key: 'pending',  label: 'Pending',  color: 'text-warn-ink' },
  { key: 'approved', label: 'Approved', color: 'text-forest'  },
  { key: 'rejected', label: 'Rejected', color: 'text-danger-ink'    },
]
  ...
  return (
    <div className="p-4 sm:p-8 max-w-5xl">
      ...
      {!comments?.length ? (
        <EmptyState
          variant="dashed"
          icon={status === 'pending' ? <CheckCircleIcon className="w-8 h-8 text-forest" strokeWidth={1.5} /> : undefined}
          ...
          {comments.map((c) => {
            ...
            return (
              <Card key={c.id} className="p-4">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  {score !== null && (
                    <span className={`text-xs font-mono ${
                      score >= 0.7 ? 'text-danger-ink' : score >= 0.4 ? 'text-warn-ink' : 'text-forest'
                    }`}>
                      {score.toFixed(2)}
                    </span>
                  )}
                ...
                {flags.length > 0 && (
                  <p className="text-xs text-danger-ink/80 mb-3">
