    ...
    <div className="fixed inset-0 z-50 bg-zinc-900/70 flex items-start justify-center pt-20 px-4" onClick={() => setOpen(false)}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {query.trim().length < 2 ? (
            <p className="text-center text-prose-faint text-sm py-8">Type at least 2 characters to search.</p>
          ) : flat.length === 0 && !loading ? (
            <p className="text-center text-prose-faint text-sm py-8">No results.</p>
          ) : (
            <>
              {results.articles.length > 0 && (
                <div className="px-2 pt-3">
                  <p className="px-2 text-xs text-prose-faint uppercase tracking-wider font-semibold mb-1">Guides</p>
              ...
              {results.reviews.length > 0 && (
                <div className="px-2 pt-3">
                  <p className="px-2 text-xs text-prose-faint uppercase tracking-wider font-semibold mb-1">Reviews</p>
              ...
              {results.media.length > 0 && (
                <div className="px-2 pt-3 pb-3">
                  <p className="px-2 text-xs text-prose-faint uppercase tracking-wider font-semibold mb-1">Media</p>
        ...
        <div className="px-4 py-2 border-t border-soft text-xs text-prose-faint shrink-0 flex items-center gap-3">
          <span><kbd className="bg-surface border border-strong rounded px-1">↑↓</kbd> nav</span>
          <span><kbd className="bg-surface border border-strong rounded px-1">↵</kbd> open</span>
          <span><kbd className="bg-surface border border-strong rounded px-1">esc</kbd> close</span>
        </div>
      </div>
    </div>
  )
