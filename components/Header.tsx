...
export default function Header({ username }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-surface-sunken/95 backdrop-blur-sm border-b border-soft/60">
              ...
              <div className="absolute right-0 top-full mt-2 w-[580px] bg-surface-sunken border border-soft rounded-2xl shadow-2xl shadow-black/60 p-5 z-50">
                ...
                <div className="mt-5 pt-4 border-t border-soft">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs text-eyebrow uppercase tracking-widest font-semibold">From {LABELS.vault.full}</p>
                    <Link
                      href="/vault"
                      onClick={() => setCatOpen(false)}
                      className="text-xs text-prose-faint hover:text-accent-text-soft font-semibold transition-colors"
                    >
                      See all →
                    </Link>
                  </div>
                  <div className="grid grid-cols-2 gap-1">
                    {VAULT_LINKS.map((v) => (
                      <Link
                        ...
                        href={v.href}
                        onClick={() => setCatOpen(false)}
                        className="flex items-start gap-2.5 p-2.5 rounded-xl hover:bg-surface transition-colors group"
                      >
                  ...
                  <div className="mt-3 pt-3 border-t border-soft/60 flex items-center justify-between flex-wrap gap-y-2">
                    <Link
                      href="/reviews"
                      onClick={() => setCatOpen(false)}
            ...
            <div ref={userMenuRef} className="hidden md:block relative">
              <button
                ...
                className={`flex items-center gap-2 p-1 pr-3 rounded-full transition-colors ${
                  userMenuOpen ? 'bg-surface-raised' : 'bg-surface hover:bg-surface-raised'
                }`}
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-surface-sunken border border-soft rounded-2xl shadow-2xl shadow-black/60 p-1.5 z-50">
                  ...
                  <div className="border-t border-soft/60 mt-1 pt-1">
      ...
      {/* Mobile drawer */}
      {mobileOpen && (
        <div className="md:hidden border-t border-soft bg-surface-sunken overflow-y-auto max-h-[calc(100dvh-4rem)]">
          {/* Main nav links */}
          ...
          <div className="px-4 pt-5 pb-2">
            <button
              ...
              className="flex items-center justify-between w-full text-xs text-eyebrow uppercase tracking-widest font-semibold mb-3"
            >
              Browse by Category
              <svg
          ...
          <div className="px-4 pb-4 border-t border-soft/40 pt-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] font-bold text-eyebrow uppercase tracking-widest">From {LABELS.vault.full}</p>
              <Link
                href="/vault"
                onClick={() => setMobileOpen(false)}
                className="text-xs text-prose-faint hover:text-accent-text-soft font-semibold transition-colors"
