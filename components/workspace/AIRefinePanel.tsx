...
export function AIRefinePanel({ title, category, content, productName, contentType, onRefined, externalInstruction, onExternalInstructionUsed }: Props) {
  ...
  async function handleRefine() {
    ...
    setError(null)
    try {
      const endpoint = contentType === 'guide' ? '/api/claude/guide-refine' : '/api/claude/review-refine'
  ...
  return (
    <div className="bg-surface border border-accent-border/30 rounded-xl p-4">
      ...
      {error && (
        <p className="mt-2 text-xs text-red-300 bg-red-950/40 border border-red-700/40 rounded-lg px-3 py-2">
          {error}
        </p>
      )}
    </div>
