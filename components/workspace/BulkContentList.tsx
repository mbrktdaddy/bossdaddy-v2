    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-4">
        <div className="relative flex-1">
          <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-prose-faint pointer-events-none" strokeWidth={2} />
          <input
            type="search"
            value={search}
                      ...
                      {contentType === 'reviews' && item.rating !== undefined && item.rating !== null
                        ...
                        : (category ? <CategoryIcon slug={category.slug} className="w-5 h-5 text-accent-text" /> : (
                          contentType === 'guides' ? (
                            ...
                            <StarIcon className="w-5 h-5 text-accent-text" strokeWidth={1.5} />
                          )
                        ))}
                    </div>
                  )}
