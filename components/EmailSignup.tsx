...
export function EmailSignup({
  ...
  if (state === 'success' || state === 'already') {
    ...
    return (
      <div className={`${isAlready ? 'bg-accent-tint border-accent-border/40' : 'bg-green-50 border-green-300'} border rounded-xl ${compact ? 'p-3' : 'p-5'}`}>
  ...
  return (
    <div className={compact ? '' : 'space-y-3'}>
      ...
      {error && (
        <p className={`${compact ? 'text-xs' : 'text-sm'} text-red-700 mt-2`}>{error}</p>
      )}
    </div>
  )
}
