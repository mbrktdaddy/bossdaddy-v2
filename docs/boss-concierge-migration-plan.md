...
>
> **PR 1 = CODE-COMPLETE (Steps 1–6).** Owed before merge (Step 7): live click-through on the Boss page (needs dev server + a Haiku-capable gateway key, i.e. pro-team key or the local `AI_MODEL_CONCIERGE_FAST=…sonnet` override). Not committed. **Then PR 2** (north-star prompt reframe + feedback loop; also folds in the coarse `SENSITIVE_HINTS` matching / gap G).
