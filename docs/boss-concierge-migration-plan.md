...
>
> **PR 2a = CODE-COMPLETE + eval-verified (branch `refactor/phase4-concierge-pr2`).** `BOSS_CONCIERGE_BASE` rewritten (item 8): intent-first routing (FIX/BUILD/HOWTO → guides first · PLAN/WRITE/TEACH/ENCOURAGE → voice · DECIDE/BUY → gear specialized · hard-limit → deflect), useful-first framing ("chief of staff, not a store"), and **cards-own-the-links** (prose stops emitting `/reviews//guides//go/` paths AND the affiliate disclosure — the card carries both). Deflect lanes + VOICE preserved **verbatim**. Backstop: `normalizeBossText` now also strips leaked internal link paths (whole-line link refs dropped; inline bare paths removed; absolute URLs spared via lookbehind) so a bare UNDISCLOSED affiliate path can never reach the reader even if the prompt drifts — the same defense-in-depth as the markdown strip. Compliance verified safe: every card carrying a buy link renders the FTC line (`RecommendationCard`, `ResearchedList`), and buy links only originate from tool results that always produce a card. **`next build` clean · unit 122 (5 new backstop tests) · golden eval 10/10** (new BARE_LINK assertion added; ran with the local `AI_MODEL_CONCIERGE_FAST=…sonnet` override — do NOT set in prod). **OWED: live click-through on `/tools/the-boss`, then push + PR onto `master`.** Then PR 2b.
