...
- **Model selection is centralized** in `lib/flags.ts` `resolveModel(bucket)` — buckets: `content`, `utility`, `moderation`, `research`, `concierge`. Override per bucket via `AI_MODEL_*` env vars; models are gateway slugs (`anthropic/claude-sonnet-5`, `anthropic/claude-haiku-4.5`, `anthropic/claude-fable-5`, `anthropic/claude-opus-5`, `xai/grok-4.5`). `moderation` is compliance-pinned to Claude (no override).
