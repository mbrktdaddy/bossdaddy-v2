...
-- /reviews and /articles all-views
create index if not exists idx_reviews_status_visible_published
  on reviews (status, is_visible, published_at desc);

create index if not exists idx_articles_status_visible_published
  on articles (status, is_visible, published_at desc);

-- Category-filtered listing variants (/reviews?category=, /articles?category=)
...
create index if not exists idx_articles_status_visible_category_published
  on articles (status, is_visible, category, published_at desc);
...
create index if not exists idx_articles_author_status_visible
  on articles (author_id, status, is_visible);
